...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        codex_linux_sandbox_exe: Some(find_linux_sandbox_exe()),
        ..ConfigOverrides::default()
    }
}

#[cfg(target_os = "linux")]
fn find_linux_sandbox_exe() -> PathBuf {
    codex_utils_cargo_bin::cargo_bin("codex-linux-sandbox")
        .ok()
        .or_else(|| find_executable_on_path("codex-linux-sandbox"))
        .expect("should find binary for codex-linux-sandbox via Cargo or PATH alias")
}
