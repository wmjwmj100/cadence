...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn durable_agent_prompts_are_sent_once_in_initial_developer_context_not_tail() {
    skip_if_no_network!();

    let server = MockServer::start().await;
    let sse = sse(vec![
        ev_response_created("resp1"),
        ev_message_item_added("msg1", "ok"),
        ev_completed("resp1"),
    ]);
    let request_log = mount_sse_sequence(&server, vec![sse.clone(), sse]).await;
    let codex_home = Arc::new(TempDir::new().unwrap());

    let prompt_dir = codex_home.path().join("agents").join("wmj-assistant");
    std::fs::create_dir_all(&prompt_dir).unwrap();
    std::fs::write(
        prompt_dir.join("manual_system_prompt.md"),
        "manual identity prompt for request test\n",
    )
    .unwrap();
    std::fs::write(
        prompt_dir.join("automatic_prompt.md"),
        "automatic stable owner boundary for request test\n",
    )
    .unwrap();
    ...
    let durable_messages = text_items
        .iter()
        .filter(|(role, text)| *role == "developer" && text.contains("<agent_durable_context"))
        .collect::<Vec<_>>();
    assert_eq!(
        durable_messages.len(),
        1,
        "durable prompt should be one developer message"
    );
    let durable_text = durable_messages[0].1;
    assert!(durable_text.contains("manual identity prompt for request test"));
    assert!(durable_text.contains("automatic stable owner boundary for request test"));
    assert!(!durable_text.contains("<runtime_tail_injection"));

    let non_developer_text = text_items
        .iter()
        .filter(|(role, _)| *role != "developer")
        .map(|(_, text)| *text)
        .collect::<Vec<_>>()
        .join("\n");
    assert!(
        !non_developer_text.contains("<agent_durable_context"),
        "durable prompt envelope must not be repeated in the runtime/history tail"
    );
    assert!(
        !non_developer_text.contains("manual identity prompt for request test"),
        "manual durable prompt must not be repeated in runtime/history messages"
    );
    assert!(
        !non_developer_text.contains("automatic stable owner boundary for request test"),
        "automatic durable prompt must not be repeated in runtime/history messages"
    );
    assert!(
        text_items
            .iter()
            .any(|(role, text)| *role == "user" && text.contains("U1")),
        "second request should still include prior user history"
    );
    assert!(
        text_items
            .iter()
            .any(|(role, text)| *role == "user" && text.contains("U2")),
        "second request should include current user input"
    );
}
