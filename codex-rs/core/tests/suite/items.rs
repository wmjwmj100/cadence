...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn termination_judge_finalize_emits_stream_info_suggestion() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let judge_response = sse(vec![
        ev_response_created("resp-judge"),
        ev_assistant_message(
            "msg-judge",
            r#"{"decision":"finalize","user_next_steps":"You can now review or run tests."}"#,
        ),
        ev_completed("resp-judge"),
    ]);
    mount_sse_once(&server, judge_response).await;
    ...
    let suggestion = wait_for_event_match(&codex, |ev| match ev {
        EventMsg::StreamInfo(StreamInfoEvent { message }) => Some(message.clone()),
        _ => None,
    })
    .await;
    assert_eq!(suggestion, "You can now review or run tests.");
