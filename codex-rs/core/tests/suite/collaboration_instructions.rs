...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn user_turn_without_collaboration_mode_keeps_existing_collaboration_instructions()
-> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let test = test_codex().build(&server).await?;
    let base_text = "persistent swarm instructions";
    let base_mode = collab_mode_with_mode_and_instructions(ModeKind::Swarm, Some(base_text));
