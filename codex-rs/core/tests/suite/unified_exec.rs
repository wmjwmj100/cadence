...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn unified_exec_terminates_explicit_failure_stall() -> Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_sandbox!(Ok(()));
    skip_if_windows!(Ok(()));
    ...
    let call_id = "uexec-explicit-failure-stall";
    let args = serde_json::json!({
        "cmd": "printf 'gmake: *** synthetic failure\\ncompilation terminated.\\n'; sleep 300",
    ...
    let failure_output = outputs.get(call_id).expect("missing failure output");
    assert!(failure_output.process_id.is_none());
    assert_eq!(failure_output.exit_code, Some(137));
    assert!(
        failure_output.output.contains("compilation terminated."),
        "expected synthetic failure text, got {:?}",
        failure_output.output
    );
    assert!(
        failure_output.wall_time_seconds < 8.0,
        "expected stalled failure to return promptly, got {}",
        failure_output.wall_time_seconds
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn unified_exec_keeps_explicit_failure_marker_while_output_continues() -> Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_sandbox!(Ok(()));
    skip_if_windows!(Ok(()));
    ...
    let first_call_id = "uexec-failure-marker-continues";
    let script = r#"python3 - <<'PY'
import sys, time
print('gmake: *** synthetic failure', flush=True)
for i in range(8):
...
PY"#;
    ...
    let second_call_id = "uexec-failure-marker-poll";
    let second_args = serde_json::json!({
        "chars": "",
        "session_id": 1000,
        "yield_time_ms": 5_000,
    ...
    codex
        .submit(Op::UserTurn {
            items: vec![UserInput::Text {
                text: "continue after noisy failure marker".into(),
                text_elements: Vec::new(),
            }],
            final_output_json_schema: None,
            cwd: cwd.path().to_path_buf(),
    ...
    let first_output = outputs.get(first_call_id).expect("missing initial output");
    assert!(first_output.process_id.is_some());
    assert!(first_output.exit_code.is_none());
    assert!(
        first_output.output.contains("gmake: *** synthetic failure"),
        "expected initial failure marker, got {:?}",
        first_output.output
    );

    let poll_output = outputs.get(second_call_id).expect("missing poll output");
    assert!(poll_output.process_id.is_none());
    assert_eq!(poll_output.exit_code, Some(0));
    assert!(
        poll_output.output.contains("done"),
        "expected continued output to complete normally, got {:?}",
        poll_output.output
    );
    assert!(
        poll_output.output.contains("tick"),
        "expected ongoing output after failure marker, got {:?}",
        poll_output.output
    );
