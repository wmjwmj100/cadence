...
pub struct Config {
    ...
    /// Whether to inject the apps developer block.
    ...
    /// event.
    ///
    /// Example `~/.wecode/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// Start the session in the specified collaboration mode (plan/default/swarm).
    ...
    /// Directory containing all Codex state (defaults to `~/.wecode` but can be
    ...
    /// Settings that govern if and what will be written to `~/.wecode/history.jsonl`.
    pub history: History,
...
/// Base config deserialized from ~/.wecode/config.toml.
...
impl Config {
    ...
    fn load_config_with_layer_stack(
        ...
        // Merge user-defined providers into the built-in list. User config wins
        // so existing custom provider ids keep working when a new built-in id is added.
...
"#,
    ...
    #[test]
    fn prompt_instruction_blocks_can_be_disabled_from_config_and_profiles() -> std::io::Result<()> {
        let codex_home = TempDir::new()?;
        let cfg = ConfigToml {
            include_permissions_instructions: Some(false),
            include_apps_instructions: Some(false),
            include_environment_context: Some(false),
            profile: Some("chatty".to_string()),
            profiles: HashMap::from([(
                "chatty".to_string(),
                ConfigProfile {
                    include_permissions_instructions: Some(true),
                    include_environment_context: Some(true),
                    ..Default::default()
                },
            )]),
            ..Default::default()
...
name = "Gradence custom"
base_url = "https://api.openai.com/v1"
env_key = "OPENAI_API_KEY"
wire_api = "responses"
request_max_retries = 15           # retry failed HTTP requests
stream_max_retries = 15            # retry dropped SSE streams
stream_idle_timeout_ms = 300000    # 5m idle timeout

[model_providers.openai-custom.profile]
...
"#;
        ...
        let openai_custom_provider = ModelProviderInfo {
            name: "Gradence custom".to_string(),
            ...
            http_headers: None,
            env_http_headers: None,
            request_max_retries: Some(15),
            stream_max_retries: Some(15),
            stream_idle_timeout_ms: Some(300_000),
            requires_openai_auth: false,
            supports_websockets: false,
            profile: crate::model_provider_info::ProviderProfile {
                compat: crate::ProviderCompatibility::ChatCompletionsAdapter,
                tool_call_style: crate::ToolCallStyle::Glm,
                history_strategy: crate::HistoryStrategy::Replay,
                heartbeat_interval_ms: Some(10_000),
                max_history_messages: Some(32),
                max_tool_output_chars: Some(8_000),
                max_message_chars: Some(16_000),
                disable_parallel_tool_calls: true,
                requires_reasoning_content_for_tool_calls: false,
                strip_unsupported_params: true,
            },
    ...
    #[test]
    fn test_builtin_kimi_model_provider_can_be_selected() -> std::io::Result<()> {
...
model_provider = "kimi"
        ...
        assert_eq!(config.model_provider.name, "Kimi Moonshot");
        assert_eq!(
        ...
        assert_eq!(
            config.model_provider.profile.compat,
            crate::model_provider_info::ProviderCompatibility::ChatCompletionsAdapter
        );
        assert_eq!(
            config.model_provider.profile.tool_call_style,
            crate::model_provider_info::ToolCallStyle::Kimi
        );
    ...
    #[test]
    fn test_user_defined_provider_overrides_builtin_kimi_provider() -> std::io::Result<()> {
        let cfg: ConfigToml = toml::from_str(
            r#"
model = "kimi-k2.6"
model_provider = "kimi"
...
base_url = "https://kimi-gateway.example/v1"
env_key = "CUSTOM_KIMI_KEY"
wire_api = "responses"

[model_providers.kimi.profile]
compat = "chat_completions_adapter"
...
history_strategy = "replay"
        ...
        assert_eq!(config.model_provider_id, "kimi");
        assert_eq!(config.model_provider.name, "Custom Kimi Gateway");
        assert_eq!(
            config.model_provider.base_url.as_deref(),
            Some("https://kimi-gateway.example/v1")
        );
        assert_eq!(
            config.model_provider.env_key.as_deref(),
            Some("CUSTOM_KIMI_KEY")
        );
        assert_eq!(
            config.model_provider.profile.compat,
            crate::model_provider_info::ProviderCompatibility::ChatCompletionsAdapter
