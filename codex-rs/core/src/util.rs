...
mod tests {
  ...
  "error": {
    ...
    fn resume_command_prefers_name_over_id() {
        ...
        assert_eq!(command, Some("wecode resume my-thread".to_string()));
    }
    ...
    fn resume_command_with_only_id() {
        ...
        assert_eq!(
            command,
            Some("wecode resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    fn resume_command_quotes_thread_name_when_needed() {
        ...
        assert_eq!(
            command,
            Some("wecode resume -- -starts-with-dash".to_string())
        );

        let command = resume_command(Some("two words"), None);
        assert_eq!(command, Some("wecode resume 'two words'".to_string()));

        let command = resume_command(Some("quote'case"), None);
        assert_eq!(command, Some("wecode resume \"quote'case\"".to_string()));
    }
}
