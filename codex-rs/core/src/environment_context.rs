...
use crate::prompt_paths::http_tool_gateway_enabled_from_env;
...
impl EnvironmentContext {
    ...
    pub fn diff(before: &TurnContext, after: &TurnContext, shell: &Shell) -> Self {
        Self::diff_for_gateway(before, after, shell, http_tool_gateway_enabled_from_env())
    }
    ...
    fn diff_for_gateway(
        ...
        let before_cwd = prompt_cwd_for_gateway(&before.cwd, gateway_enabled);
        let after_cwd = prompt_cwd_for_gateway(&after.cwd, gateway_enabled);
        let before_network = Self::network_from_turn_context(before);
        let after_network = Self::network_from_turn_context(after);
        let cwd = if before_cwd != after_cwd {
            Some(after_cwd)
        } else {
            None
        };
    ...
    pub fn from_turn_context(turn_context: &TurnContext, shell: &Shell) -> Self {
        Self::from_turn_context_with_cwd(
            turn_context,
            shell,
    ...
    fn from_turn_context_with_cwd(turn_context: &TurnContext, shell: &Shell, cwd: PathBuf) -> Self {
        Self::new(
            Some(cwd),
            shell.clone(),
