...
use crate::agent::refresh_agent_automatic_prompt_once_per_day_for_agent;
...
pub(crate) fn start_memories_startup_task(
    ...
    tokio::spawn(async move {
        ...
        if let Err(err) = refresh_agent_automatic_prompt_once_per_day_for_agent(
            &config.codex_home,
            PRIMARY_AGENT_NAME,
            PRIMARY_AGENT_NAME,
        )
        .await
        {
            warn!("failed to refresh agent automatic prompt: {err}");
        }

        if session.services.state_db.is_none() {
            warn!("state db unavailable for memories startup pipeline; skipping phase extraction");
            return;
        }
