...
pub(super) async fn run(session: &Arc<Session>, config: Arc<Config>) {
    ...
    let thread_id = match session
        .services
        .agent_control
        .spawn_agent(agent_config, Some(source))
        .await
    {
    ...
    if let Err(err) = session
        .services
        .agent_control
        .send_input(thread_id, prompt)
        .await
    {
        tracing::error!("failed to start global memory consolidation agent: {err}");
        let _ = session
            .services
            .agent_control
            .shutdown_agent(thread_id)
            .await;
        job::failed(session, db, &claim, "failed_send_input").await;
        return;
    }
