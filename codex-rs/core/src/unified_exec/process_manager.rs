...
use crate::exec_env::insert_agent_name_env;
...
const EXPLICIT_FAILURE_STALL_GRACE: Duration = Duration::from_secs(3);
const EXPLICIT_FAILURE_STALL_EXIT_CODE: i32 = 137;
const FAILURE_SCAN_WINDOW_BYTES: usize = 8 * 1024;
...
impl UnifiedExecProcessManager {
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        insert_agent_name_env(
            &mut env,
            context
                .session
                .services
                .agent_control
                .agent_name_for_thread(context.session.conversation_id)
                .as_deref(),
        );
    ...
    pub(super) async fn collect_output_until_deadline(
        ...
        let mut explicit_failure_seen = false;
        ...
        loop {
            ...
            if drained_chunks.is_empty() {
                ...
                {
                    let idle = Instant::now().saturating_duration_since(last_output_at);
                    if idle >= EXPLICIT_FAILURE_STALL_GRACE {
                        process.terminate_with_exit_code(EXPLICIT_FAILURE_STALL_EXIT_CODE);
                        break;
                    }
                }
                ...
                let failure_wait = if explicit_failure_seen {
                    EXPLICIT_FAILURE_STALL_GRACE
                        .saturating_sub(Instant::now().saturating_duration_since(last_output_at))
                } else {
                    remaining
                };
                let sleep_for = remaining.min(failure_wait);
                tokio::select! {
                    _ = &mut notified => {}
            ...
            for chunk in drained_chunks {
                ...
                Self::push_failure_scan_window(&mut recent_output, &chunk);
            }

            explicit_failure_seen |= Self::output_indicates_explicit_failure(&recent_output);
    ...
    fn push_failure_scan_window(window: &mut Vec<u8>, chunk: &[u8]) {
        window.extend_from_slice(chunk);
        if window.len() > FAILURE_SCAN_WINDOW_BYTES {
            let overflow = window.len() - FAILURE_SCAN_WINDOW_BYTES;
            window.drain(..overflow);
        }
    }

    fn output_indicates_explicit_failure(window: &[u8]) -> bool {
