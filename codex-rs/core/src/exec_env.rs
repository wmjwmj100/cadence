...
pub const CODEX_AGENT_NAME_ENV_VAR: &str = "CODEX_AGENT_NAME";
...
pub fn insert_agent_name_env(env_map: &mut HashMap<String, String>, agent_name: Option<&str>) {
    if let Some(name) = agent_name.map(str::trim)
        && !name.is_empty()
    {
        env_map.insert(CODEX_AGENT_NAME_ENV_VAR.to_string(), name.to_string());
    }
}
...
mod tests {
    ...
    #[test]
    fn insert_agent_name_env_sets_variable_for_non_empty_name() {
        let mut env = HashMap::new();
        insert_agent_name_env(&mut env, Some("Jordan-explorer"));
        assert_eq!(
            env.get(CODEX_AGENT_NAME_ENV_VAR),
            Some(&"Jordan-explorer".to_string())
        );
    }

    #[test]
    fn insert_agent_name_env_ignores_empty_name() {
        let mut env = HashMap::new();
        insert_agent_name_env(&mut env, Some("   "));
        assert!(!env.contains_key(CODEX_AGENT_NAME_ENV_VAR));
    }
