...
pub enum CodexErr {
    ...
    /// Returned when a model stream cannot be started, or produces no first event, before the turn
    /// has emitted any model output. This remains globally non-retryable so broad pre-first-event
    /// failures do not spend the entire benchmark timeout budget silently reconnecting; selected
    /// startup timeouts may be retried by narrower caller logic.
    #[error("model response failed before first event: {0}")]
    InitialResponseFailed(String),

    #[error(
        "wecode ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    #[error(
        "To use wecode with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
...
impl CodexErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            | CodexErr::InitialResponseFailed(_)
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/wecode/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using wecode (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/wecode/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
mod tests {
    ...
    fn usage_limit_reached_error_formats_plus_plan() {
        ...
        assert_eq!(
            err.to_string(),
            "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/wecode/settings/usage to purchase more credits or try again later."
        );
    }
    ...
    fn usage_limit_reached_error_formats_free_plan() {
        ...
        assert_eq!(
            err.to_string(),
            "You've hit your usage limit. Upgrade to Plus to continue using wecode (https://chatgpt.com/explore/plus), or try again later."
        );
    }
    ...
    fn usage_limit_reached_error_formats_pro_plan_with_reset() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Visit https://chatgpt.com/wecode/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
    ...
    fn usage_limit_reached_includes_hours_and_minutes() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/wecode/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
    ...
    fn usage_limit_reached_with_promo_message() {
        ...
        with_now_override(base, move || {
            ...
            let err = UsageLimitReachedError {
                ...
                rate_limits: Some(Box::new(rate_limit_snapshot())),
                promo_message: Some(
                    "To continue using wecode, start a free trial of <PLAN> today".to_string(),
                ),
            };
            let expected = format!(
                "You've hit your usage limit. To continue using wecode, start a free trial of <PLAN> today, or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
