...
mod slice {
    ...
    use crate::tools::handlers::read_file::slice_truncation_notice;
    use std::path::Path;
    use tokio::fs::File;
    ...
    pub async fn read(
        ...
        let mut seen = 0usize;
        let mut last_returned_line = None;
        ...
        if let Some(last_line) = last_returned_line {
            if last_line < seen {
                collected.push(slice_truncation_notice(last_line, seen, limit));
            }
        }
...
mod indentation {
    ...
    use crate::tools::handlers::read_file::format_line;
    use crate::tools::handlers::read_file::indentation_truncation_notice;
    ...
    pub async fn read_block(
        ...
        let truncated_by_cap = out.len() == final_limit && (i >= 0 || j < collected.len());
        ...
        if truncated_by_cap {
            rendered.push(indentation_truncation_notice(
                anchor_line,
                collected.len(),
                final_limit,
            ));
...
fn slice_truncation_notice(last_line: usize, total_lines: usize, limit: usize) -> String {
    format!(
        "Note: file continues past line {last_line}; file has {total_lines} total lines. Call read_file with offset={} limit={limit} to read more.",
        last_line + 1,
    )
}

fn indentation_truncation_notice(
    anchor_line: usize,
    file_total_lines: usize,
    line_cap: usize,
) -> String {
    format!(
        "Note: indentation mode output was capped at {line_cap} lines around anchor line {anchor_line}; file has {file_total_lines} total lines. Increase limit or indentation.max_lines to expand further."
    )
}
...
"
        ...
        assert_eq!(
            ...
            vec![
                "L1: first".to_string(),
                "L2: second".to_string(),
                "Note: file continues past line 2; file has 3 total lines. Call read_file with offset=3 limit=2 to read more.".to_string(),
            ]
        );
        Ok(())
    }
...
"
    ...
    #[tokio::test]
    async fn indentation_mode_reports_when_block_is_capped() -> anyhow::Result<()> {
        let mut temp = NamedTempFile::new()?;
        use std::io::Write as _;
        write!(
            temp,
...
"
        ...
        let lines = read_block(temp.path(), 3, 3, options).await?;
        assert_eq!(
            lines,
            vec![
                "L2:     if first {".to_string(),
                "L3:         a();".to_string(),
                "L4:     }".to_string(),
                "Note: indentation mode output was capped at 3 lines around anchor line 3; file has 8 total lines. Increase limit or indentation.max_lines to expand further.".to_string(),
            ]
        );
        Ok(())
    }
