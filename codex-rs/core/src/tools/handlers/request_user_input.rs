...
mod tests {
    ...
    fn request_user_input_mode_availability_is_plan_only() {
        ...
        assert!(!ModeKind::Swarm.allows_request_user_input());
    ...
    fn request_user_input_unavailable_messages_use_default_name_for_default_modes() {
        ...
        assert_eq!(
            request_user_input_unavailable_message(ModeKind::Swarm),
            Some("request_user_input is unavailable in Swarm mode".to_string())
        );
        assert_eq!(
