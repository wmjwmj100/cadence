...
pub(crate) struct ToolsConfig {
    ...
    pub collab_tool_set: CollabToolSet,
...
impl ToolsConfig {
    pub fn new(params: &ToolsConfigParams) -> Self {
        ...
        let include_collab_tools =
            features.enabled(Feature::Collab) || matches!(mode_kind, ModeKind::Swarm);
        let include_collaboration_modes_tools = features.enabled(Feature::CollaborationModes);
        let request_rule_enabled = features.enabled(Feature::RequestRule);
        let include_search_tool = features.enabled(Feature::Apps);
        let collab_tool_set = match mode_kind {
            ModeKind::Swarm => CollabToolSet::Swarm,
            ModeKind::Default | ModeKind::Plan | ModeKind::Execute | ModeKind::PairProgramming => {
                CollabToolSet::Full
            }
        };
...
fn create_wait_tool() -> ToolSpec {
    ...
    properties.insert(
        ...
        JsonSchema::Number {
            description: Some(
                "Optional total time to wait in milliseconds. If target_id points to an Agent and timeout_ms is omitted, wait defaults to 7 minutes. Human targets suspend until a relevant message or interrupt arrives. Plain wait without target_id still requires timeout_ms."
                    .to_string(),
            ),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(
                "Optional target participant id. For Agent targets this is the agent name or thread id; wait notifies that Agent and only consumes messages from that target. Human ids are represented by the same field at the office layer."
                    .to_string(),
            ),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(
                "Legacy alias for target_id. Prefer target_id for new call/wait designs."
                    .to_string(),
            ),
        },
    );
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: "wait".to_string(),
        description: "Pause execution until your inbox has at least one relevant message. Plain wait preserves FIFO inbox semantics and requires timeout_ms. wait(target_id) notifies the target Agent when applicable, defaults Agent waits to 7 minutes when timeout_ms is omitted, and only consumes messages from that target so unrelated Agent/Human messages stay queued. When a message arrives, the result is only that message's content. When the wait times out, the result is the structured timeout JSON with timedOut, timeoutStreak, haltRequired, and suggestion."
            .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
fn create_call_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "target_id".to_string(),
            JsonSchema::String {
                description: Some(
                    "Target participant id to message. For existing Swarm Agents this is the agent name or thread id returned from spawn_agent; office Human targets use the same field at the product layer."
                        .to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Legacy alias for target_id when targeting a Swarm Agent. Prefer target_id for new call/wait designs."
                        .to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Message content. CRITICAL: When delegating tasks, DO NOT send vague one-liners. You MUST provide: 1) Context, 2) Goal/Start Point, 3) MECE Boundaries (what NOT to do), and 4) Strict Output Contract (e.g., JSON or Markdown table)."
                        .to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Caller-chosen unique ID for this message. Format: <sender>-<receiver>-<one-word-topic>, for example `avery-jordan-history`. Use a new message_id for each new call. Replies must reference this via reply_to_message_id."
                        .to_string(),
                ),
            },
        ),
        (
            "need_reply".to_string(),
            JsonSchema::Boolean {
                description: Some(
                    "Set to true if you expect the target agent to execute work and call back with results. Set to false for FYI/Silent updates."
                        .to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(
                    "CRITICAL FOR RESOLVING WAITS: If you are answering a request, set this to the original message_id provided by the caller."
                        .to_string(),
                ),
            },
        ),
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: "call".to_string(),
        description: "Send input or delegate tasks to another participant. Existing Swarm Agent targets are dispatched by agent name/thread id: if idle, a new turn starts; if running, input is queued. The same message_id, need_reply, and reply_to_message_id semantics are reused for office Human targets."
            .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
            required: Some(vec!["content".to_string(), "message_id".to_string()]),
            additional_properties: Some(false.into()),
        },
    })
}

fn create_read_agent_status_tool() -> ToolSpec {
    let properties = BTreeMap::from([(
        "agent_id".to_string(),
        JsonSchema::String {
            description: Some(
                "Target agent identifier. Must exactly match an existing agent id or registered agent name."
                    .to_string(),
            ),
        },
    )]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: "read_agent_status".to_string(),
        description: "Read the recent activity and external state of a target agent. The status field is limited to idle, working, or waiting; waiting includes a natural-language reason when available. Returns the latest 10 entries (tool_call and assistant messages combined). Assistant messages are truncated to 100 characters to provide a summary view."
            .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
fn create_spawn_agent_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Required for high-quality delegation. Define identity instead of a concrete task: include a strong Role, a Persona/Backstory (temperament, standards, style), and a narrow professional Goal boundary (the agent's sub-problem, not the global mission)."
                        .to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(format!(
                    "Agent archetype ({}). Prefer passing this explicitly: 'explorer' for reconnaissance and mapping, 'worker' for execution and edits, 'verifier' for read-only acceptance checks and artifact validation.",
        ...
        (
            ...
            JsonSchema::String {
                description: Some(
                    "A brief, human-readable description of what this sub-agent is created to do."
                        .to_string(),
                ),
            },
        ),
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description:
            "Create a specialized sub-agent and return its name for future call/wait. Does NOT start execution immediately. Always configure a strong system_prompt for better results."
                .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
fn create_send_input_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "id".to_string(),
            JsonSchema::String {
                description: Some("Agent id or name to message (from spawn_agent).".to_string()),
            },
        ),
        (
            "message".to_string(),
...
fn create_resume_agent_tool() -> ToolSpec {
    ...
    properties.insert(
        "id".to_string(),
        JsonSchema::String {
            description: Some("Agent id or name to resume.".to_string()),
        },
    );
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description:
            "Resume a previously closed agent by id or name so it can receive send_input and wait calls."
                .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
fn create_close_agent_tool() -> ToolSpec {
    ...
    properties.insert(
        "id".to_string(),
        JsonSchema::String {
            description: Some("Agent id or name to close (from spawn_agent).".to_string()),
        },
    );
...
pub(crate) fn build_specs(
    ...
    use crate::tools::handlers::CallHandler;
    ...
    use crate::tools::handlers::ReadAgentStatusHandler;
    ...
    if config.collab_tools {
        ...
        let call_handler = Arc::new(CallHandler);
        let read_agent_status_handler = Arc::new(ReadAgentStatusHandler);
        ...
        builder.push_spec_with_parallel_support(create_read_agent_status_tool(), true);
        builder.register_handler("read_agent_status", read_agent_status_handler);
        match config.collab_tool_set {
            CollabToolSet::Full => {
                ...
                builder.register_handler("wait", wait_handler);
        ...
        builder.register_handler("call", call_handler);
        builder.push_spec_with_parallel_support(create_call_tool(), true);
    }
...
mod tests {
    ...
    fn strip_descriptions_tool(spec: &mut ToolSpec) {
        match spec {
            ToolSpec::Function(ResponsesApiTool { parameters, .. }) => {
                ...
                strip_summary_requirement(parameters);
            }
            ToolSpec::Freeform(_) | ToolSpec::LocalShell {} | ToolSpec::WebSearch { .. } => {}
        }
    }

    fn strip_summary_requirement(schema: &mut JsonSchema) {
        ...
        properties.remove(crate::tools::TOOL_SUMMARY_ARGUMENT);
        if let Some(required_fields) = required {
            required_fields.retain(|name| name != crate::tools::TOOL_SUMMARY_ARGUMENT);
        }
        if required.as_ref().is_some_and(std::vec::Vec::is_empty) {
            *required = None;
        }
    }

    fn assert_tool_spec_eq_ignoring_summary(actual: &ToolSpec, expected: &ToolSpec) {
        let mut actual = actual.clone();
        let mut expected = expected.clone();
        strip_descriptions_tool(&mut actual);
        strip_descriptions_tool(&mut expected);
    ...
    #[test]
    fn test_build_specs_swarm_mode_only_exposes_spawn_call_wait_and_remind() {
        let config = test_config();
        let model_info =
            ModelsManager::construct_model_info_offline_for_tests("gpt-5-codex", &config);
        let mut features = Features::with_defaults();
        ...
        let tools_config = ToolsConfig::new(&ToolsConfigParams {
            ...
            features: &features,
            web_search_mode: Some(WebSearchMode::Cached),
            mode_kind: ModeKind::Swarm,
        });
        let (tools, _) = build_specs(&tools_config, None, None, &[]).build();
        assert_contains_tool_names(
            &tools,
            &["spawn_agent", "call", "wait", "read_agent_status"],
        );
        let names = tools.iter().map(|t| t.spec.name()).collect::<Vec<_>>();
        for blocked in ["send_input", "resume_agent", "close_agent"] {
            assert!(
                !names.contains(&blocked),
                "expected tool {blocked} to be absent; had: {names:?}"
            );
        }
    }

    #[test]
    fn swarm_call_wait_schema_supports_target_id_and_optional_wait_timeout() {
        let config = test_config();
        let model_info =
            ModelsManager::construct_model_info_offline_for_tests("gpt-5-codex", &config);
        let mut features = Features::with_defaults();
        ...
        let call = tools
            .iter()
            .find(|tool| tool.spec.name() == "call")
            .expect("call tool");
        let ToolSpec::Function(call_spec) = &call.spec else {
            panic!("call should be a function tool");
        };
        let JsonSchema::Object {
            properties,
            required,
            ...
            panic!("call parameters should be object");
        };
        assert!(properties.contains_key("target_id"));
        assert!(properties.contains_key("target_agent_name"));
        let required = required.as_ref().expect("call should have required fields");
        assert!(required.contains(&"content".to_string()));
        assert!(required.contains(&"message_id".to_string()));
        assert!(required.contains(&"summary".to_string()));
        assert!(!required.contains(&"target_id".to_string()));
        assert!(!required.contains(&"target_agent_name".to_string()));

        let wait = tools
            .iter()
            .find(|tool| tool.spec.name() == "wait")
            .expect("wait tool");
        let ToolSpec::Function(wait_spec) = &wait.spec else {
            panic!("wait should be a function tool");
        };
        let JsonSchema::Object {
            properties,
            required,
            ...
            panic!("wait parameters should be object");
        };
        assert!(properties.contains_key("target_id"));
        assert!(properties.contains_key("target_agent_id"));
        let required = required.as_ref().expect("wait should require summary only");
        assert_eq!(required, &["summary".to_string()]);
    }

    #[test]
    fn test_swarm_mode_enables_spawn_and_call_without_collab_feature_flag() {
        let config = test_config();
        let model_info =
            ModelsManager::construct_model_info_offline_for_tests("gpt-5-codex", &config);
        let mut features = Features::with_defaults();
    ...
    fn test_build_specs_mcp_tools_converted() {
        ...
        assert_tool_spec_eq_ignoring_summary(
            &tool.spec,
            &ToolSpec::Function(ResponsesApiTool {
...
- Always set the `workdir` param when using the shell_command function. Do not use `cd` unless absolutely necessary."#.to_string()
    ...
    #[test]
    fn json_schema_object_serializes_required_before_properties() {
        let actual = serde_json::to_string(&JsonSchema::Object {
        ...
        let required_idx = actual
            .find("\"required\"")
            .expect("required key should exist");
        let properties_idx = actual
            .find("\"properties\"")
            .expect("properties key should exist");

        assert!(
            required_idx < properties_idx,
            "required should serialize before properties: {actual}"
        );
    }
