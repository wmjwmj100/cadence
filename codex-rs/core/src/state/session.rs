...
pub(crate) struct SessionState {
    ...
    recent_real_user_inputs: VecDeque<String>,
}
...
impl SessionState {
    ...
    pub(crate) fn new(session_configuration: SessionConfiguration) -> Self {
        ...
        Self {
            ...
            recent_real_user_inputs: VecDeque::new(),
        }
    }

    pub(crate) fn push_recent_real_user_input(&mut self, input: String) {
        const MAX_RECENT_REAL_USER_INPUTS: usize = 3;

        if self.recent_real_user_inputs.len() == MAX_RECENT_REAL_USER_INPUTS {
            self.recent_real_user_inputs.pop_front();
        }
        self.recent_real_user_inputs.push_back(input);
    }

    pub(crate) fn recent_real_user_inputs(&self) -> Vec<String> {
        self.recent_real_user_inputs.iter().cloned().collect()
    }
    ...
    pub(crate) fn set_shared_blackboard_path(&mut self, path: PathBuf) {
        self.shared_blackboard_path = Some(path);
    }

    pub(crate) fn shared_blackboard_path(&self) -> Option<PathBuf> {
        self.shared_blackboard_path.clone()
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn recent_real_user_inputs_keeps_last_three_entries() {
        let session_configuration = make_session_configuration_for_tests().await;
        let mut state = SessionState::new(session_configuration);

        state.push_recent_real_user_input("first".to_string());
        state.push_recent_real_user_input("second".to_string());
        state.push_recent_real_user_input("third".to_string());
        state.push_recent_real_user_input("fourth".to_string());
