...
impl ThreadManagerState {
    ...
    /// List all currently registered thread ids.
    pub(crate) async fn list_thread_ids(&self) -> Vec<ThreadId> {
        self.threads.read().await.keys().copied().collect()
    }

    /// List active thread IDs for sub-agents that were spawned by `parent_thread_id`.
    pub(crate) async fn list_thread_ids_spawned_by(
        &self,
        ...
        for (thread_id, thread) in threads {
            ...
            if let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
                ...
                && actual_parent == parent_thread_id
            {
                spawned.push(thread_id);
            }
        }
        spawned
    }
