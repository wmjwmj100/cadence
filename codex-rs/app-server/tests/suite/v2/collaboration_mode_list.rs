...
async fn list_collaboration_modes_returns_presets() -> Result<()> {
    ...
    let expected = vec![swarm_preset(), swarm_complex_preset()];
...
fn swarm_preset() -> CollaborationModeMask {
    ...
    presets
        .into_iter()
        .find(|p| p.mode == Some(ModeKind::Swarm) && p.name == "Swarm")
        .unwrap()
}
...
fn swarm_complex_preset() -> CollaborationModeMask {
    ...
    presets
        .into_iter()
        .find(|p| p.mode == Some(ModeKind::Swarm) && p.name == "Swarm Complex")
        .unwrap()
}
