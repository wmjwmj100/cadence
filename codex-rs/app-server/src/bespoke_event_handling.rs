...
use codex_app_server_protocol::DebugTraceSnapshot as V2DebugTraceSnapshot;
...
use codex_core::protocol::DebugTraceSnapshotEvent;
    ...
    match msg {
        ...
        EventMsg::CollabWaitingBegin(begin_event) => {
            let receiver_thread_ids = begin_event
                .targets
                .iter()
                .map(|target| target.receiver_thread_id.to_string())
                .collect();
            let item = ThreadItem::CollabAgentToolCall {
                id: begin_event.call_id,
        ...
        EventMsg::CollabWaitingEnd(end_event) => {
            let status = if end_event.timed_out
                || end_event.targets.iter().any(|target| {
                    matches!(
                        target.state,
                        CollabWaitLifecycleState::Failed | CollabWaitLifecycleState::TimedOut
                    )
                }) {
            ...
            let agents_states = end_event
                ...
                .map(|target| {
                    let state = match target.state {
                        CollabWaitLifecycleState::Pending => V2CollabAgentStatus {
                            status: codex_app_server_protocol::CollabAgentStatus::PendingInit,
                            message: None,
                        },
                        CollabWaitLifecycleState::Running => V2CollabAgentStatus {
                            status: codex_app_server_protocol::CollabAgentStatus::Running,
                            message: None,
                        },
                        CollabWaitLifecycleState::Completed => V2CollabAgentStatus {
                            status: codex_app_server_protocol::CollabAgentStatus::Completed,
                            message: target.callback_content.clone(),
                        },
                        CollabWaitLifecycleState::TimedOut => V2CollabAgentStatus {
                            status: codex_app_server_protocol::CollabAgentStatus::Running,
                            message: None,
                        },
                        CollabWaitLifecycleState::Failed => V2CollabAgentStatus {
                            status: codex_app_server_protocol::CollabAgentStatus::Errored,
                            message: target.callback_content.clone(),
                        },
                    };
                    (target.receiver_thread_id.to_string(), state)
                })
                .collect();
            let item = ThreadItem::CollabAgentToolCall {
                id: end_event.call_id,
        ...
        EventMsg::DebugTraceSnapshot(trace_event) => {
            handle_debug_trace_snapshot(
                conversation_id,
                &event_turn_id,
                trace_event,
                api_version,
...
async fn handle_debug_trace_snapshot(
    conversation_id: ThreadId,
    event_turn_id: &str,
    trace_event: DebugTraceSnapshotEvent,
    api_version: ApiVersion,
    outgoing: &ThreadScopedOutgoingMessageSender,
) {
    if let ApiVersion::V2 = api_version {
        let entry_count = trace_event.snapshot.entries.len();
        let notification = ItemCompletedNotification {
            thread_id: conversation_id.to_string(),
            turn_id: event_turn_id.to_string(),
            item: ThreadItem::DebugTraceSnapshot {
                id: format!(
                    "debug-trace-{}-{entry_count}",
                    trace_event.snapshot.generated_at_unix_sec
                ),
                trace: V2DebugTraceSnapshot::from(trace_event.snapshot),
            },
        };
        outgoing
            .send_server_notification(ServerNotification::ItemCompleted(notification))
...
mod tests {
    ...
    use codex_core::protocol::DebugTraceAgentKind as CoreDebugTraceAgentKind;
    use codex_core::protocol::DebugTraceEntry as CoreDebugTraceEntry;
    use codex_core::protocol::DebugTraceLane as CoreDebugTraceLane;
    use codex_core::protocol::DebugTraceLanePosition as CoreDebugTraceLanePosition;
    use codex_core::protocol::DebugTraceRole as CoreDebugTraceRole;
    use codex_core::protocol::DebugTraceSnapshot as CoreDebugTraceSnapshot;
    use codex_core::protocol::DebugTraceSnapshotEvent as CoreDebugTraceSnapshotEvent;
    ...
    #[tokio::test]
    async fn test_handle_debug_trace_snapshot_emits_item_completed_for_v2() -> Result<()> {
        let (tx, mut rx) = mpsc::channel(CHANNEL_CAPACITY);
        let outgoing = Arc::new(OutgoingMessageSender::new(tx));
        let outgoing = ThreadScopedOutgoingMessageSender::new(outgoing, vec![ConnectionId(1)]);
        let conversation_id = ThreadId::new();
        let trace_event = CoreDebugTraceSnapshotEvent {
            conversation_id,
            snapshot: CoreDebugTraceSnapshot {
        ...
        match msg {
            OutgoingMessage::AppServerNotification(ServerNotification::ItemCompleted(n)) => {
                ...
                match n.item {
                    ThreadItem::DebugTraceSnapshot { id, trace } => {
                        assert_eq!(id, "debug-trace-10-1");
                        assert_eq!(trace.schema_version, "debug_trace_v1");
                        assert_eq!(trace.entries.len(), 1);
                        assert_eq!(trace.entries[0].agent_name, "wmj-assistant");
                    }
    ...
    #[tokio::test]
    async fn test_handle_debug_trace_snapshot_is_noop_for_v1() -> Result<()> {
        let (tx, mut rx) = mpsc::channel(CHANNEL_CAPACITY);
        let outgoing = Arc::new(OutgoingMessageSender::new(tx));
        let outgoing = ThreadScopedOutgoingMessageSender::new(outgoing, vec![ConnectionId(1)]);
        let conversation_id = ThreadId::new();
