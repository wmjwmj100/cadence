# Wecode MCP Server Interface [experimental]

This document describes Wecode’s experimental MCP server interface: a JSON‑RPC API that runs over the Model Context Protocol (MCP) transport to control a local Wecode engine.

- Status: experimental and subject to change without notice
- Server binary: `wecode mcp-server` (or `wecode-mcp-server`)
- Transport: standard MCP over stdio (JSON‑RPC 2.0, line‑delimited)

## Overview

Wecode exposes a small set of MCP‑compatible methods to create and manage conversations, send user input, receive live events, and handle approval prompts. The types are defined in `protocol/src/mcp_protocol.rs` and re‑used by the MCP server implementation in `mcp-server/`.

At a glance:

- Conversations
  - `newConversation` → start a Wecode session
...
Run Wecode as an MCP server and connect an MCP client:

```bash
wecode mcp-server | your_mcp_client
```
...
Use the separate `wecode mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
Fetch the catalog of models available in the current Wecode build with `model/list`. The request accepts optional pagination inputs:
...
- `codex/event` with the serialized Wecode event payload. The shape matches `core/src/protocol.rs`’s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
compatibility with MCP clients that prefer `structuredContent`, Wecode mirrors the
...
```json
{
  "content": [{ "type": "text", "text": "Hello from Wecode" }],
  "structuredContent": {
    "threadId": "019bbed6-1e9e-7f31-984c-a05b65045719",
    "content": "Hello from Wecode"
  }
}
```
...
When Wecode needs approval to apply changes or run commands, the server issues JSON‑RPC requests to the client:
...
```json
{ "jsonrpc": "2.0", "id": 2, "method": "sendUserMessage", "params": { "conversationId": "c7b0…", "items": [{ "type": "text", "text": "Hello Wecode" }] } }
```
