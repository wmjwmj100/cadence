...
const LARGE_WORDMARK_WIDTH: u16 = 49;
const LARGE_WORDMARK: [&str; 7] = [
    r"##     ## #######  #####   #####  ######  #######",
    r"##     ## ##      ##   ## ##   ## ##   ## ##     ",
    r"##  #  ## ##      ##      ##   ## ##   ## ##     ",
    r"## ### ## #####   ##      ##   ## ##   ## #####  ",
...
impl WidgetRef for &WelcomeWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        if layout_area.width >= LARGE_WORDMARK_WIDTH {
            let left_pad = usize::from(layout_area.width.saturating_sub(LARGE_WORDMARK_WIDTH) / 2);
            lines.extend(
                LARGE_WORDMARK
                    .iter()
                    .map(|line| Line::from(format!("{}{}", " ".repeat(left_pad), line)).bold()),
            );
            lines.push("".into());
        } else {
            lines.push(Line::from(vec!["  ".into(), "WECODE".bold()]));
            lines.push("".into());
        }
        if show_animation {
        ...
        lines.push(Line::from(vec![
            ...
            "Welcome to ".into(),
            "wecode".bold(),
            " :: the Gradence coding cockpit".into(),
        ]));
        lines.push(Line::from(vec![
            "  ".into(),
            "Ship changes, steer tools, and shape the workspace from one terminal.".dim(),
        ]));
        lines.push("".into());
        lines.push(Line::from(vec!["  Tips for getting started:".dim()]));
        lines.push(Line::from(vec![
            "  1. ".dim(),
            "Ask questions, edit files, or run commands.".dim(),
        ]));
        lines.push(Line::from(vec![
            "  2. ".dim(),
            "Be specific for the best results.".dim(),
        ]));
        lines.push(Line::from(vec![
            "  3. ".dim(),
            "/help".cyan(),
...
mod tests {
    ...
    fn welcome_renders_animation_on_first_draw() {
        ...
        let banner_row = row_containing(&buf, "##     ## #######");
        let welcome_row = row_containing(&buf, "Welcome");
        assert_eq!(banner_row, Some(0));
        assert_eq!(welcome_row, Some(frame_lines + 11));
    }
