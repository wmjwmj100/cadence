...
fn test_session_configured_event(
    session_id: ThreadId,
) -> codex_core::protocol::SessionConfiguredEvent {
    codex_core::protocol::SessionConfiguredEvent {
        session_id,
        forked_from_id: None,
        thread_name: None,
...
fn swarm_mask() -> CollaborationModeMask {
...
async fn make_chatwidget_manual(
    ...
    let mut bottom = BottomPane::new(BottomPaneParams {
        ...
        enhanced_keys_supported: false,
        placeholder_text: "Ask wecode to inspect, edit, test, or explain".to_string(),
...
async fn exec_end_without_begin_uses_event_command() {
    ...
    assert!(
        blob.contains("• shell echo orphaned"),
...
async fn exec_history_shows_unified_exec_startup_commands() {
    ...
    let blob = lines_to_single_string(&cells[0]);
    assert!(
        blob.contains("• shell echo unified exec startup"),
...
async fn collab_mode_shift_tab_cycles_only_when_enabled_and_idle() {
    ...
    let expected_after_first_cycle = collaboration_modes::next_mask(
        chat.models_manager.as_ref(),
        chat.active_collaboration_mask.as_ref(),
    )
    .and_then(|mask| mask.mode)
    .expect("expected a visible collaboration mode");
    chat.handle_key_event(KeyEvent::from(KeyCode::BackTab));
    assert_eq!(
        chat.active_collaboration_mode_kind(),
        expected_after_first_cycle
    );
    assert_eq!(chat.current_collaboration_mode(), &initial);

    let expected_after_second_cycle = collaboration_modes::next_mask(
        chat.models_manager.as_ref(),
        chat.active_collaboration_mask.as_ref(),
    )
    .and_then(|mask| mask.mode)
...
#[tokio::test]
async fn collab_slash_command_complex_switches_to_complex_swarm() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.thread_id = Some(ThreadId::new());
    chat.set_feature_enabled(Feature::CollaborationModes, true);

    chat.dispatch_command_with_args(SlashCommand::Collab, "complex".to_string(), Vec::new());
...
#[tokio::test]
async fn status_widget_swarm_subagent_progress_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.set_feature_enabled(Feature::CollaborationModes, true);

    let primary_thread = ThreadId::new();
    chat.handle_codex_event(Event {
        id: "configured".into(),
        msg: EventMsg::SessionConfigured(test_session_configured_event(primary_thread)),
    });
    chat.set_collaboration_mask(swarm_mask());
    ...
    chat.handle_codex_event(Event {
        id: "task-1".into(),
        msg: EventMsg::AgentReasoningDelta(AgentReasoningDeltaEvent {
            delta: "**Clarifying swarm mode collaboration strategy**".into(),
        }),
    });

    for (agent_name, summary) in [
        ("john_assistant", "Inspecting wait queue behavior"),
        ("taylor_assistant", "Validating call tool response history"),
        ("alex_assistant", "Reviewing tool summary extraction"),
        ("morgan_assistant", "Drafting snapshot coverage"),
        ("casey_assistant", "Checking ordering and cap behavior"),
        ("riley_assistant", "Cross-checking overflow behavior"),
    ] {
        chat.handle_codex_event(Event {
            id: "agent-work".into(),
            msg: EventMsg::AgentWorkSummary(AgentWorkSummaryEvent {
                thread_id: ThreadId::new(),
    ...
    let height = chat.desired_height(100);
    let mut terminal = ratatui::Terminal::new(ratatui::backend::TestBackend::new(100, height))
        .expect("create terminal");
    terminal
        .draw(|f| chat.render(f.area(), f.buffer_mut()))
        .expect("draw status widget");
    assert_snapshot!("status_widget_swarm_subagent_progress", terminal.backend());
}

#[tokio::test]
async fn subagent_progress_reorders_and_caps_to_five() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(None).await;
    chat.set_feature_enabled(Feature::CollaborationModes, true);
    ...
    let thread_ids: Vec<ThreadId> = (0..6).map(|_| ThreadId::new()).collect();
    for (index, thread_id) in thread_ids.iter().enumerate() {
        let ordinal = index + 1;
        chat.handle_codex_event(Event {
            id: format!("agent-work-{ordinal}"),
            msg: EventMsg::AgentWorkSummary(AgentWorkSummaryEvent {
                thread_id: *thread_id,
                agent_name: format!("agent_{ordinal}"),
                summary: format!("summary {ordinal}"),
            }),
        });
    }

    let names = chat
        .subagent_progress_entries
        .iter()
    ...
    chat.handle_codex_event(Event {
        id: "agent-work-reorder".into(),
        msg: EventMsg::AgentWorkSummary(AgentWorkSummaryEvent {
            thread_id: thread_ids[2],
            agent_name: "agent_3".to_string(),
            summary: "summary 3 updated".to_string(),
        }),
    });
    chat.handle_codex_event(Event {
        id: "agent-work-reenter".into(),
        msg: EventMsg::AgentWorkSummary(AgentWorkSummaryEvent {
            thread_id: thread_ids[0],
            agent_name: "agent_1".to_string(),
            summary: "summary 1 updated".to_string(),
        }),
    });

    let reordered_names = chat
        .subagent_progress_entries
        .iter()
        .map(|entry| entry.agent_name.clone())
        .collect::<Vec<_>>();
    ...
    let details = chat
        ...
        .expect("status details should be shown");
...
async fn final_reasoning_then_message_without_deltas_are_rendered() {
    ...
    chat.handle_codex_event(Event {
        ...
        msg: EventMsg::AgentMessage(AgentMessageEvent {
            ...
            summary: "Here is the result.".into(),
        }),
    });
...
async fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    chat.handle_codex_event(Event {
        ...
        msg: EventMsg::AgentMessage(AgentMessageEvent {
            ...
            summary: "I’m going to search the repo for where “Change Approved” is rendered to update that view.".into(),
        }),
    });
