...
const BASE_BUG_ISSUE_URL: &str =
    "https://github.com/gradence/wecode/issues/new?template=2-bug-report.yml";
/// Internal routing link for employee feedback follow-ups. This must not be shown to external users.
const CODEX_FEEDBACK_INTERNAL_URL: &str = "http://go/wecode-feedback-internal";
            ...
            Ok(()) => {
                ...
                let mut lines = vec![Line::from(match issue_url.as_ref() {
                    Some(_) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        format!("{prefix} Please report this in #wecode-feedback:")
                    }
                ...
                match issue_url {
                    Some(url) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        lines.extend([
                            ...
                            Line::from(vec![
                                "    ".into(),
                                format!("https://go/wecode-feedback/{thread_id}").bold(),
                            ]),
                        ]);
                    }
                    Some(url) => {
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current wecode session logs with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    fn issue_url_available_for_bug_bad_result_and_other() {
        ...
        let expected_slack_url = "http://go/wecode-feedback-internal".to_string();
