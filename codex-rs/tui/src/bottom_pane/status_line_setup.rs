...
impl StatusLineItem {
    ...
    pub(crate) fn description(&self) -> &'static str {
        match self {
            ...
            StatusLineItem::CodexVersion => "wecode application version",
    ...
    pub(crate) fn new(status_line_items: Option<&[String]>, app_event_tx: AppEventSender) -> Self {
        ...
        Self {
            picker: MultiSelectPicker::builder(
                ...
                Some("Choose the signals that stay visible under the input panel.".to_string()),
                app_event_tx,
            )
            .instructions(vec![
                "Use up/down to move, left/right to reorder, space to toggle, enter to save, esc to cancel."
                    .into(),
            ])
            .items(items)
            .enable_ordering()
            .on_preview(|items| {
                ...
                if preview.is_empty() {
                    ...
                    Some(Line::from(format!("Status  {preview}")))
                }
            })
