...
const COMPOSER_BRAND_LABEL: &str = " WECODE ";
...
impl ChatComposer {
    ...
    pub(crate) fn new_with_config(
        ...
        let mut this = Self {
            ...
            is_zellij: codex_core::terminal::terminal_info().is_zellij(),
            config,
...
impl ChatComposer {
    pub(crate) fn render_with_mask(&self, area: Rect, buf: &mut Buffer, mask_char: Option<char>) {
        ...
        if composer_rect.width > 4 {
            let top_width = composer_rect.width.saturating_sub(2) as usize;
            let label_fits = top_width > COMPOSER_BRAND_LABEL.len();
            let top_rule = if label_fits {
                let remainder = top_width.saturating_sub(COMPOSER_BRAND_LABEL.len());
                Line::from(vec![
                    Span::styled(COMPOSER_BRAND_LABEL, accent_style()),
                    Span::styled("─".repeat(remainder), Style::default().fg(Color::DarkGray)),
                ])
            } else {
                Line::from(vec![Span::styled("─".repeat(top_width), accent_style())])
            };
            top_rule.render(
                Rect {
                    x: composer_rect.x + 1,
                    y: composer_rect.y,
                    width: composer_rect.width.saturating_sub(2),
                    height: 1,
                },
                buf,
            );
        ...
        if !textarea_rect.is_empty() {
            let prompt = if self.input_enabled {
                Span::styled(COMPOSER_PROMPT_PREFIX, accent_style())
            } else {
            ...
            if !textarea_rect.is_empty() {
                if is_zellij {
                    buf.set_string(
                        ...
                        textarea_rect.y,
                        text,
                        textarea_style.italic(),
                    );
                } else {
                    let placeholder = Span::styled(text, Style::default().fg(Color::DarkGray));
...
mod tests {
    ...
    fn footer_hint_row_is_separated_from_composer() {
        ...
        let composer = ChatComposer::new(
            ...
            sender,
            false,
            "Ask wecode to inspect, edit, test, or explain".to_string(),
            false,
        );
        ...
        assert!(
            !spacing_row.contains("? help"),
            "expected composer body above hints, not a duplicated footer row: {spacing_row:?}",
        );
    }
    ...
    fn zellij_empty_composer_snapshot() {
        snapshot_zellij_composer_state("zellij_empty_composer", |_| {});
    }
