...
const SEARCH_PLACEHOLDER: &str = "type to filter commands and settings";
...
impl Renderable for MultiSelectPicker {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if content_area.width > 4 {
            Line::from(vec!["[ ".into(), "Command Panel".bold(), " ]".into()])
                .style(accent_style())
                .render(
                    Rect {
                        x: content_area.x + 1,
        ...
        if search_area.height >= 2 {
            ...
            Line::from(vec!["Search  ".into(), SEARCH_PLACEHOLDER.dim()])
        ...
        let hint_area = if let Some(preview_line) = &self.preview_line {
            ...
            let preview_line = truncate_line_with_ellipsis_if_overflow(
                Line::from(vec![
                    "Preview  ".bold(),
                    Span::from(preview_line.clone().to_string()),
                ]),
                max_preview_width,
            );
...
impl MultiSelectPickerBuilder {
    ...
    pub fn build(self) -> MultiSelectPicker {
        ...
        header.push(Line::from(vec!["== ".into(), self.title.clone().bold()]));

        if let Some(subtitle) = self.subtitle {
            header.push(Line::from(vec!["  grid :: ".dim(), subtitle.dim()]));
        }
