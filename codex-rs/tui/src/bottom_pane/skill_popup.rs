...
mod tests {
    ...
    fn mention_item(display_name: &str, search_terms: &[&str]) -> MentionItem {
        MentionItem {
    ...
    fn display_name_match_sorting_beats_worse_secondary_search_term_matches() {
        let mut popup = SkillPopup::new(vec![
            mention_item("pr-review-triage", &["pr-review-triage"]),
            mention_item("prd", &["prd"]),
            mention_item("PR Babysitter", &["babysit-pr", "PR Babysitter"]),
            mention_item("Plugin Creator", &["plugin-creator", "Plugin Creator"]),
            mention_item(
