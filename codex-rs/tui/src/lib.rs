...
// The standalone `wecode-tui` binary prints a short help message before the
...
pub async fn run_main(
    ...
    if complex_swarm_enabled_from_env() {
        cli.config_overrides
            .raw_overrides
            .push("tui.experimental_mode=\"swarm\"".to_string());
    }
    ...
    let codex_home = match find_codex_home() {
        ...
        Err(err) => {
            eprintln!("Error finding wecode home: {err}");
            std::process::exit(1);
        }
    };
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `wecode {action}` without an ID to choose from existing sessions."
            )),
        })
    };
