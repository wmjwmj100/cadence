...
impl HistoryCell for UserHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let label = Line::from(vec![
            Span::styled(" USER ", accent_style()),
            Span::from("  live input").dim(),
        ])
        .style(style);
        let mut lines: Vec<Line<'static>> = vec![label, Line::from("").style(style)];
...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            line![
                "See ",
                "https://github.com/gradence/wecode".cyan().underlined(),
        ...
        let content = text![
            ...
            "https://github.com/gradence/wecode/releases/latest"
                .cyan()
                .underlined(),
        ];
        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    "You ".into(),
                    "approved".bold(),
                    " wecode to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
...
const SESSION_HEADER_BANNER_WIDTH: usize = 49;
const SESSION_HEADER_BANNER: [&str; 7] = [
    r"##     ## #######  #####   #####  ######  #######",
    r"##     ## ##      ##   ## ##   ## ##   ## ##     ",
    r"##  #  ## ##      ##      ##   ## ##   ## ##     ",
    r"## ### ## #####   ##      ##   ## ##   ## #####  ",
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for wecode".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what wecode is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let deck_chip = |label: &'static str| Span::styled(format!(" {label} "), accent_style());
        let show_banner = inner_width >= SESSION_HEADER_BANNER_WIDTH;

        let title_spans: Vec<Span<'static>> = vec![
            Span::from("[ wecode cli ]").bold(),
            Span::from("  "),
            Span::styled("[ ready ]", accent_style()),
            Span::from("  "),
            Span::from("command deck").dim(),
            Span::from("  ").into(),
            Span::from(format!("v{}", self.version)).dim(),
        ];
        let subtitle_spans: Vec<Span<'static>> = vec![
            Span::from("  ask anything").dim(),
            Span::from("  ::  ").dim(),
            Span::from("ship code from one terminal").dim(),
        ];
        ...
        let model_spans: Vec<Span<'static>> = {
            ...
            if let Some(reasoning) = reasoning_label {
                ...
                spans.push(Span::styled(format!("[{reasoning}]"), accent_style()));
            }
            spans.push(Span::from("  "));
        ...
        let dir_prefix_width = UnicodeWidthStr::width("ROOT ");
        let dir_max_width = inner_width.saturating_sub(dir_prefix_width);
        let dir = self.format_directory(Some(dir_max_width));
        let dir_spans = vec![deck_chip("ROOT"), Span::from(" "), Span::from(dir)];
        let quick_spans = vec![
            deck_chip("NAV"),
            Span::from(" "),
            "tab agents".cyan(),
            Span::from("  "),
            "ctrl+p commands".cyan(),
            Span::from("  "),
            "/model".cyan(),
        ];
        ...
        if show_banner {
            let left_pad = inner_width.saturating_sub(SESSION_HEADER_BANNER_WIDTH) / 2;
            banner_lines.extend(SESSION_HEADER_BANNER.iter().map(|line| {
                make_row(vec![Span::styled(
        ...
        lines.extend([
            ...
            make_row(subtitle_spans),
            make_row(Vec::new()),
            make_row(model_spans),
            make_row(dir_spans),
            make_row(quick_spans),
            make_row(Vec::new()),
            make_row(vec![
                Span::from("Tips  1. ask clearly  2. attach context  3. /help").dim(),
            ]),
        ]);

        let mut out = banner_lines;
        if !out.is_empty() {
            out.push(make_row(Vec::new()));
        }
        out.extend(with_border(lines));
        out
    }
}
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://developers.gradence.com/wecode/mcp\u{7}MCP docs\u{1b}]8;;\u{7}"
                .underlined(),
...
impl HistoryCell for ProposedPlanCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(
            vec![
                Span::styled(" PLAN ", accent_style()),
                Span::from("  execution outline").dim(),
            ]
            .into(),
        );
        lines.push(Line::from(" "));
...
mod tests {
    ...
    fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
        ...
        assert_eq!(
            rendered,
            vec![
                "✔ You approved wecode to".to_string(),
    ...
    #[test]
    fn session_header_includes_quick_menu_commands() {
        let cell =
            SessionHeaderHistoryCell::new("gpt-4o".to_string(), None, std::env::temp_dir(), "test");

        let lines = render_lines(&cell.display_lines(100));
        let menu_line = lines
            .into_iter()
            .find(|line| line.contains("ctrl+p commands"))
            .expect("menu line");

        assert!(menu_line.contains("tab agents"));
        assert!(menu_line.contains("/model"));
    }

    #[test]
    fn session_header_shows_banner_when_width_allows() {
        let cell =
            SessionHeaderHistoryCell::new("gpt-4o".to_string(), None, std::env::temp_dir(), "test");
        ...
        assert!(
            ...
            "expected session header banner to render for wide layouts"
        );
        ...
        let title_index = lines
            .iter()
            .position(|line| line.contains("command deck"))
            .expect("session title line");
        assert!(
            banner_index == 0,
            "expected session header banner to render before any bordered card content"
        );
        assert!(
            banner_index < title_index,
            "expected session header banner to sit outside the card border"
        );
    }
