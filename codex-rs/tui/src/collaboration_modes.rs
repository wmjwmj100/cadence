...
pub(crate) fn find_mask(
    ...
    predicate: impl Fn(&CollaborationModeMask) -> bool,
) -> Option<CollaborationModeMask> {
    filtered_presets(models_manager).into_iter().find(predicate)
}
...
pub(crate) fn complex_swarm_mask(models_manager: &ModelsManager) -> Option<CollaborationModeMask> {
    find_mask(models_manager, |mask| {
        mask.mode == Some(ModeKind::Swarm) && mask.name == "Swarm Complex"
    })
}
...
pub(crate) fn next_mask(
    ...
    let next_index = presets
        .iter()
        .position(|mask| current.is_some_and(|current_mask| current_mask == mask))
