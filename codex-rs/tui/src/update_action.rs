...
pub enum UpdateAction {
    /// Update via `npm install -g @gradence/wecode@latest`.
    NpmGlobalLatest,
    /// Update via `bun install -g @gradence/wecode@latest`.
    BunGlobalLatest,
    /// Update via `brew upgrade wecode`.
    BrewUpgrade,
}
...
impl UpdateAction {
    ...
    pub fn command_args(self) -> (&'static str, &'static [&'static str]) {
        match self {
            UpdateAction::NpmGlobalLatest => ("npm", &["install", "-g", "@gradence/wecode"]),
            UpdateAction::BunGlobalLatest => ("bun", &["install", "-g", "@gradence/wecode"]),
            UpdateAction::BrewUpgrade => ("brew", &["upgrade", "--cask", "wecode"]),
        }
    }
