...
use codex_core::protocol::AgentWorkSummaryEvent;
...
const MAX_VISIBLE_SUBAGENT_PROGRESS: usize = 5;
const SUBAGENT_PROGRESS_SUMMARY_PREVIEW_GRAPHEMES: usize = 80;
...
use codex_core::swarm::complex_swarm_enabled_from_env;
mod interrupts;
...
pub(crate) struct ChatWidget {
    ...
    // Most-recent-first sub-agent progress rows shown under the status header in swarm mode.
    subagent_progress_entries: Vec<SubagentProgressEntry>,
...
impl ChatWidget {
    ...
    fn set_status(&mut self, header: String, details: Option<String>) {
        ...
        let details = self.combined_status_details(details);
        self.bottom_pane.update_status(header, details);
    }

    fn combined_status_details(&self, base_details: Option<String>) -> Option<String> {
        let base_details = base_details
            .map(|details| details.trim().to_string())
            .filter(|details| !details.is_empty());
        let progress_details = self.subagent_progress_details();
        match (base_details, progress_details) {
            (Some(base), Some(progress)) => Some(format!("{base}\n{progress}")),
    ...
    fn should_show_subagent_progress(&self) -> bool {
        self.agent_turn_running && self.active_mode_kind() == ModeKind::Swarm
    }

    fn subagent_progress_details(&self) -> Option<String> {
        if !self.should_show_subagent_progress() || self.subagent_progress_entries.is_empty() {
            return None;
        }
        Some(
            self.subagent_progress_entries
                .iter()
                .take(MAX_VISIBLE_SUBAGENT_PROGRESS)
                .map(|entry| format!("{}: {}", entry.agent_name, entry.summary))
                .collect::<Vec<_>>()
                .join("\n"),
        )
    }

    fn refresh_status_indicator_details(&mut self) {
        let details = self.combined_status_details(None);
        self.bottom_pane
            .update_status(self.current_status_header.clone(), details);
    }

    fn on_agent_work_summary(&mut self, event: AgentWorkSummaryEvent) {
        if self.thread_id == Some(event.thread_id) {
            return;
        }

        let trimmed = event.summary.trim();
        let summary = if trimmed.is_empty() {
            "working...".to_string()
        } else {
            truncate_text(trimmed, SUBAGENT_PROGRESS_SUMMARY_PREVIEW_GRAPHEMES)
        };

        self.subagent_progress_entries
            .retain(|entry| entry.thread_id != event.thread_id);
        self.subagent_progress_entries.insert(
            0,
        ...
        if self.subagent_progress_entries.len() > MAX_VISIBLE_SUBAGENT_PROGRESS {
            self.subagent_progress_entries
                .truncate(MAX_VISIBLE_SUBAGENT_PROGRESS);
        }

        self.refresh_status_indicator_details();
        self.request_redraw();
    }
    ...
    fn on_task_started(&mut self) {
        ...
        self.subagent_progress_entries.clear();
    ...
    fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "wecode is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        let mut widget = Self {
            ...
            subagent_progress_entries: Vec::new(),
        ...
        match cmd {
            ...
            SlashCommand::Status => {
                if self.should_prefetch_rate_limits() {
                    ...
                    self.next_status_refresh_request_id = request_id.saturating_add(1);
                    ...
                    tokio::spawn(async move {
                        let result = if let Some(auth) = auth_manager.auth().await {
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Collab if !trimmed.is_empty() => {
                ...
                let mask = match lowered.as_str() {
                    "complex" | "swarm-complex" | "complex-swarm" => {
                        collaboration_modes::complex_swarm_mask(self.models_manager.as_ref())
                    }
                    "default" | "standard" | "swarm" => {
                ...
                if let Some(mask) = mask {
                    ...
                    self.add_info_message(
                        "Requested collaboration preset is unavailable right now.".to_string(),
                        None,
                    );
                }
            }
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        ...
        match msg {
            ...
            EventMsg::AgentWorkSummary(ev) => {
                if !from_replay {
                    self.on_agent_work_summary(ev);
                }
            }
    ...
    fn open_personality_popup_for_current_model(&mut self) {
        ...
        header.push(Line::from("Choose a communication style for wecode.".dim()));
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running wecode -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_collaboration_modes_popup(&mut self) {
        ...
        let items: Vec<SelectionItem> = presets
            ...
            .map(|mask| {
                ...
                let is_current = current_kind.as_ref() == Some(&mask);
                let actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
    ...
    pub(crate) fn open_full_access_confirmation(
        ...
        let info_line = Line::from(vec![
            "When wecode runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        if !codex_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED {
            ...
            header.push(*Box::new(
                Paragraph::new(vec![
                    ...
                    line!["Learn more: https://developers.gradence.com/wecode/windows"],
                ])
        ...
        header.push(*Box::new(
            Paragraph::new(vec![
                line!["Set up the wecode agent sandbox to protect your files and control network access. Learn more <https://developers.gradence.com/wecode/windows>"],
            ])
            .wrap(Wrap { trim: false }),
        ));
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(&mut self, preset: ApprovalPreset) {
        ...
        lines.push(line![""]);
        lines.push(line![
            "You can still use wecode in a non-admin sandbox. It carries greater risk if prompt injected."
        ]);
        lines.push(line![
            "Learn more <https://developers.gradence.com/wecode/windows>"
        ]);
        ...
        let items = vec![
            ...
            SelectionItem {
                name: "Use wecode with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.otel_manager.clone();
    ...
    fn initial_collaboration_mask(
        ...
        let mut mask = match config.experimental_mode {
            Some(ModeKind::Swarm) if complex_swarm_enabled_from_env() => {
                collaboration_modes::complex_swarm_mask(models_manager).or_else(|| {
                    collaboration_modes::mask_for_kind(models_manager, ModeKind::Swarm)
                })?
            }
    ...
    fn collaboration_mode_indicator(&self) -> Option<CollaborationModeIndicator> {
        ...
        match self.active_mode_kind() {
            ...
            ModeKind::Default | ModeKind::Swarm | ModeKind::PairProgramming | ModeKind::Execute => {
                None
            }
        }
    }
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = codex_core::util::resume_command(Some(name), thread_id)
            .unwrap_or_else(|| format!("wecode resume {name}"));
    ...
    fn connectors_popup_params(&self, connectors: &[connectors::AppInfo]) -> SelectionViewParams {
        ...
        for connector in connectors {
            ...
            let instructions = if connector.is_accessible {
                ...
                "Install this app in your browser, then reload wecode."
            };
...
const PLACEHOLDERS: [&str; 8] = [
    "Ask wecode to fix broken tests",
    "Ask wecode to review this diff",
    "Ask wecode to trace this production bug",
    "Build {feature} with tests",
    "Refactor @filename safely",
    "Improve docs for @filename",
    "Run /review on my working tree",
    "Open /skills and suggest a workflow",
];
