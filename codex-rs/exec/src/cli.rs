...
pub struct Cli {
    ...
    /// Allow running wecode outside a Git repository.
    ...
    /// Start the session in swarm collaboration mode (enabled by default for exec).
    #[arg(long = "swarm", global = true, default_value_t = true)]
    pub swarm: bool,
...
mod tests {
    ...
    #[test]
    fn parses_swarm_flag() {
        let cli = Cli::parse_from(["codex-exec", "--swarm", "hello"]);
        assert!(cli.swarm);
        assert_eq!(cli.prompt.as_deref(), Some("hello"));
    }

    #[test]
    fn defaults_to_swarm_mode() {
        let cli = Cli::parse_from(["codex-exec", "hello"]);
        assert!(cli.swarm);
        assert_eq!(cli.prompt.as_deref(), Some("hello"));
    }
}
