...
use crate::exec_events::TurnLivenessEvent;
...
impl EventProcessorWithJsonOutput {
    ...
    pub fn collect_thread_events(&mut self, event: &protocol::Event) -> Vec<ThreadEvent> {
        match &event.msg {
            ...
            protocol::EventMsg::TurnDiff(ev) => {
                // Accumulate the unified diff
                self.accumulated_unified_diff.push_str(&ev.unified_diff);
                Vec::new()
            }
            ...
            protocol::EventMsg::BackgroundEvent(ev) => self.handle_background_event(ev),
    ...
    fn handle_collab_wait_end(&mut self, ev: &CollabWaitingEndEvent) -> Vec<ThreadEvent> {
        let status = if ev.timed_out
            || ev.targets.iter().any(|target| {
                matches!(
                    target.state,
        ...
        let agents_states = ev
            ...
            .map(|target| {
                let state = match target.state {
                    CollabWaitLifecycleState::Pending => CollabAgentState {
                        status: CollabAgentStatus::PendingInit,
                        message: None,
                    },
                    CollabWaitLifecycleState::Running => CollabAgentState {
                        status: CollabAgentStatus::Running,
                        message: None,
                    },
                    CollabWaitLifecycleState::Completed => CollabAgentState {
                        status: CollabAgentStatus::Completed,
                        message: target.callback_content.clone(),
                    },
                    CollabWaitLifecycleState::TimedOut => CollabAgentState {
                        status: CollabAgentStatus::Running,
                        message: None,
                    },
                    CollabWaitLifecycleState::Failed => CollabAgentState {
    ...
    fn handle_background_event(&mut self, ev: &protocol::BackgroundEventEvent) -> Vec<ThreadEvent> {
        let stage = ev.stage.as_deref().or_else(|| match ev.message.as_str() {
            "Still waiting for model response stream to start..." => Some("stream_start_wait"),
            "Still waiting for first model response event..." => Some("first_response_event_wait"),
            _ => None,
        });

        match stage {
            Some(stage) => vec![ThreadEvent::TurnLiveness(TurnLivenessEvent {
