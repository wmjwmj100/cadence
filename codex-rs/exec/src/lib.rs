...
pub async fn run_main(cli: Cli, codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("codex_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set wecode exec originator override {err:?}"
        );
    }
    ...
    // Print the effective configuration and initial request so users can see what wecode
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);

    info!("wecode initialized with event: {session_configured:?}");
...

    {
        ...
        tokio::spawn(async move {
            if tokio::signal::ctrl_c().await.is_ok() {
                ...
                // Immediately notify wecode to abort any in-flight task.
