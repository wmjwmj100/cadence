...
use crate::event_processor::handle_unified_diff;
...
pub(crate) struct EventProcessorWithHumanOutput {
    ...
    /// Accumulated unified diff for file changes during the turn.
    accumulated_unified_diff: String,
}
    ...
    pub(crate) fn create_with_ansi(
        ...
        if with_ansi {
            Self {
                ...
                accumulated_unified_diff: String::new(),
            }
        } else {
            Self {
                call_id_to_patch,
...
impl EventProcessor for EventProcessorWithHumanOutput {
    ...
    fn print_config_summary(
        ...
        ts_msg!(
            self,
            "Gradence wecode v{} (research preview)\n--------",
            VERSION
        );
    ...
    fn process_event(&mut self, event: Event) -> CodexStatus {
        ...
        match msg {
            ...
            EventMsg::StreamInfo(codex_core::protocol::StreamInfoEvent { message }) => {
                ts_msg!(self, "{} {message}", "info:".style(self.dimmed));
            }
            ...
            EventMsg::TurnComplete(TurnCompleteEvent {
                ...
                if let Some(output_file) = self.last_message_path.as_deref() {
                    // Output unified diff instead of last message
                    if !self.accumulated_unified_diff.is_empty() {
                        handle_unified_diff(&self.accumulated_unified_diff, output_file);
                    } else {
                        // Fallback to last message if no diff available
            ...
            EventMsg::AgentMessage(AgentMessageEvent { message, .. }) => {
                ts_msg!(
                    self,
                    "{}\n{}",
                    "wecode".style(self.italic).style(self.magenta),
                    message,
                );
            }
            ...
            EventMsg::TurnDiff(TurnDiffEvent { unified_diff }) => {
                // Accumulate the unified diff for output to file
                self.accumulated_unified_diff.push_str(&unified_diff);
            ...
            EventMsg::SessionConfigured(session_configured_event) => {
                ...
                ts_msg!(
                    self,
                    "{} {}",
                    "wecode session".style(self.magenta).style(self.bold),
            ...
            EventMsg::CollabWaitingBegin(CollabWaitingBeginEvent {
                ...
                let sender = if sender_agent_name.trim().is_empty() {
                    sender_thread_id.to_string()
                } else {
                    sender_agent_name.trim().to_string()
                };
                let receivers = if targets.is_empty() {
                    "none".to_string()
                } else {
                    targets
                        .iter()
                        .map(wait_target_receiver)
                        .collect::<Vec<_>>()
                        .join(", ")
                };
                eprintln!("  sender: {}", sender.style(self.dimmed));
                eprintln!("  receivers: {}", receivers.style(self.dimmed));
            }
            EventMsg::CollabWaitingEnd(CollabWaitingEndEvent {
                ...
                let has_failed_targets = targets
                    .iter()
                    .any(|target| matches!(&target.state, CollabWaitLifecycleState::Failed));
                let title_style = if timed_out || has_failed_targets {
                    self.red
                } else {
                    self.green
                };
                ...
                eprintln!("  sender: {}", sender.style(self.dimmed));
                if targets.is_empty() {
                    eprintln!("  targets: {}", "none".style(self.dimmed));
                    return CodexStatus::Running;
                }
                let mut sorted = targets.into_iter().collect::<Vec<_>>();
                sorted.sort_by(|left, right| {
                    (
                        wait_target_receiver(left).to_ascii_lowercase(),
                        left.receiver_thread_id.to_string(),
                        left.message_id.clone(),
                    )
                        .cmp(&(
                            wait_target_receiver(right).to_ascii_lowercase(),
                            right.receiver_thread_id.to_string(),
                            right.message_id.clone(),
                        ))
                });
                for target in sorted {
                    let (state_text, state_style) = match &target.state {
                        CollabWaitLifecycleState::Pending => ("pending", self.dimmed),
                        CollabWaitLifecycleState::Running => ("running", self.cyan),
                        CollabWaitLifecycleState::Completed => ("completed", self.green),
                        CollabWaitLifecycleState::TimedOut => ("timed out", self.yellow),
                        CollabWaitLifecycleState::Failed => ("failed", self.red),
                    };
                    ...
                    {
                        let preview = truncate_preview(callback_content, 120);
                        eprintln!(
                            "  {} [{}] {}: \"{}\"",
                            wait_target_receiver(&target).style(self.dimmed),
                            target.message_id.style(self.dimmed),
                            state_text.style(state_style),
                            preview.style(self.dimmed),
                        );
                    } else {
                        eprintln!(
                            "  {} [{}] {}",
...
fn wait_target_receiver(target: &CollabWaitTargetEvent) -> String {
    let receiver_agent_name = target.receiver_agent_name.trim();
    if receiver_agent_name.is_empty() {
        target.receiver_thread_id.to_string()
    } else {
        receiver_agent_name.to_string()
    }
}
