...
pub(crate) fn handle_unified_diff(unified_diff: &str, output_file: &Path) {
    write_last_message_file(unified_diff, Some(output_file));
    if unified_diff.is_empty() {
        eprintln!(
            "Warning: no unified diff; wrote empty content to {}",
            output_file.display()
        );
    }
}
