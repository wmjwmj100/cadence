...
use codex_exec::exec_events::TurnLivenessEvent;
...
#[test]
fn startup_background_event_produces_turn_liveness_event() {
    let mut ep = EventProcessorWithJsonOutput::new(None);
    let out = ep.collect_thread_events(&event(
        "bg1",
        EventMsg::BackgroundEvent(BackgroundEventEvent {
            message: "Still waiting for model response stream to start...".to_string(),
            stage: None,
            elapsed_ms: None,
            timeout_ms: None,
            outcome: None,
    ...
    assert_eq!(
        out,
        vec![ThreadEvent::TurnLiveness(TurnLivenessEvent {
            stage: "stream_start_wait".to_string(),
            message: "Still waiting for model response stream to start...".to_string(),
            elapsed_ms: None,
            timeout_ms: None,
            outcome: None,
...
#[test]
fn unrelated_background_event_is_ignored() {
    let mut ep = EventProcessorWithJsonOutput::new(None);
    let out = ep.collect_thread_events(&event(
        "bg2",
        EventMsg::BackgroundEvent(BackgroundEventEvent {
            message: "background cleanup finished".to_string(),
            stage: None,
            elapsed_ms: None,
            timeout_ms: None,
            outcome: None,
...
#[test]
fn collab_wait_begin_uses_target_receiver_ids() {
    let mut ep = EventProcessorWithJsonOutput::new(None);
    let sender_thread_id = ThreadId::from_string("67e55044-10b1-426f-9247-bb680e5fe0c8").unwrap();
    let receiver_a = ThreadId::from_string("3f76d2a0-943e-4f43-8a38-b289c9c6c3d1").unwrap();
    let receiver_b = ThreadId::from_string("c1dfd96e-1f0c-4f26-9b4f-1aa02c2d3c4d").unwrap();
        ...
        EventMsg::CollabWaitingBegin(CollabWaitingBeginEvent {
            ...
            targets: vec![
                CollabWaitTargetEvent {
                    ...
                    receiver_agent_name: "agent-a".to_string(),
                ...
                CollabWaitTargetEvent {
                    ...
                    receiver_agent_name: "agent-b".to_string(),
        ...
        vec![ThreadEvent::ItemStarted(ItemStartedEvent {
            item: ThreadItem {
                ...
                details: ThreadItemDetails::CollabToolCall(CollabToolCallItem {
                    ...
                    sender_thread_id: sender_thread_id.to_string(),
                    receiver_thread_ids: vec![receiver_a.to_string(), receiver_b.to_string()],
                    prompt: None,
                    agents_states: std::collections::HashMap::new(),
                    status: CollabToolCallStatus::InProgress,
                }),
...
fn collab_wait_end_without_begin_synthesizes_failed_item() {
    let mut ep = EventProcessorWithJsonOutput::new(None);
    let sender_thread_id = ThreadId::from_string("67e55044-10b1-426f-9247-bb680e5fe0c8").unwrap();
    let sender_agent_name = "wmj-assistant".to_string();
    ...
    let targets = vec![
        CollabWaitTargetEvent {
            receiver_thread_id: running_thread_id,
            receiver_agent_name: "agent-running".to_string(),
            message_id: "m-running".to_string(),
            state: CollabWaitLifecycleState::Completed,
            callback_content: Some("done".to_string()),
        },
        CollabWaitTargetEvent {
            receiver_thread_id: failed_thread_id,
            receiver_agent_name: "agent-failed".to_string(),
            message_id: "m-failed".to_string(),
            state: CollabWaitLifecycleState::Failed,
            callback_content: Some("boom".to_string()),
        },
    ];
...
#[test]
fn collab_wait_end_timed_out_marks_failed_and_running_target() {
    let mut ep = EventProcessorWithJsonOutput::new(None);
    let sender_thread_id = ThreadId::from_string("67e55044-10b1-426f-9247-bb680e5fe0c8").unwrap();
    let timed_out_thread_id =
        ThreadId::from_string("4fe5f5db-87ed-43e2-aaef-b95d7b2715b7").unwrap();
    ...
    let end = event(
        ...
        EventMsg::CollabWaitingEnd(CollabWaitingEndEvent {
            ...
            targets: vec![CollabWaitTargetEvent {
                receiver_thread_id: timed_out_thread_id,
                receiver_agent_name: "agent-timeout".to_string(),
                message_id: "m-timeout".to_string(),
        ...
        vec![ThreadEvent::ItemCompleted(ItemCompletedEvent {
            item: ThreadItem {
                ...
                details: ThreadItemDetails::CollabToolCall(CollabToolCallItem {
                    tool: CollabTool::Wait,
                    sender_thread_id: sender_thread_id.to_string(),
                    receiver_thread_ids: vec![timed_out_thread_id.to_string()],
                    prompt: None,
                    agents_states: [(
                        timed_out_thread_id.to_string(),
                        CollabAgentState {
                            status: CollabAgentStatus::Running,
                            message: None,
                        },
