...
#[clap(
    ...
    // `wecode-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `wecode` command name that users run.
    bin_name = "wecode",
    override_usage = "wecode [OPTIONS] [PROMPT]\n       wecode [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for wecode.
    Mcp(McpCli),

    /// Start wecode as an MCP server (stdio).
    McpServer,

    /// [experimental] Run the app server or related tooling.
    AppServer(AppServerCommand),

    /// Launch the wecode desktop app (downloads the macOS installer if missing).
    #[cfg(target_os = "macos")]
    ...
    /// Run commands within a wecode-provided sandbox.
    Sandbox(SandboxArgs),
    ...
    /// Apply the latest diff produced by wecode agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from wecode Cloud and apply changes locally.
...
enum DebugSubcommand {
    /// Print the exact model-visible input items for a turn.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv GRADENCE_API_KEY | wecode login --with-api-key`)"
    )]
    with_api_key: bool,
...
struct AppServerCommand {
    ...
    /// ```
    ///
    /// See https://developers.gradence.com/wecode/config-advanced/#metrics for more details.
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    {
        let resume_cmd = resume_cmd.replacen("codex ", "wecode ", 1);
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating wecode via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart wecode.");
    Ok(())
}
...
async fn run_debug_prompt_input_command(
    ...
    if let Some(prompt) = cmd.prompt.or(interactive.prompt) {
        input.push(codex_protocol::user_input::UserInput::Text {
            text: prompt.replace("\r\n", "\n").replace('\r', "\n"),
...
async fn cli_main(codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    if interactive.swarm {
        root_config_overrides
            .raw_overrides
            .push("tui.experimental_mode=\"swarm\"".to_string());
    }
    ...
    match subcommand {
        ...
        Some(Subcommand::Review(review_args)) => {
            let mut exec_cli = ExecCli::try_parse_from(["wecode", "exec"])?;
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv GRADENCE_API_KEY | wecode login --with-api-key`."
                        );
                        std::process::exit(1);
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". wecode's interactive TUI may not work in this terminal."
        );
...
fn merge_interactive_cli_flags(interactive: &mut TuiCli, subcommand_cli: TuiCli) {
    ...
    if subcommand_cli.swarm {
        interactive.swarm = true;
    }
...
mod tests {
    ...
    #[test]
    fn exec_defaults_to_swarm_mode() {
        let cli = MultitoolCli::try_parse_from(["codex", "exec", "hello"]).expect("parse");
        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        assert!(exec.swarm);
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run wecode resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run wecode resume my-thread".to_string(),
            ]
        );
    }
    ...
    #[test]
    fn resume_swarm_flag_applies() {
        let interactive = finalize_resume_from_args(["codex", "resume", "--swarm"].as_ref());
        assert!(interactive.swarm);
    }
