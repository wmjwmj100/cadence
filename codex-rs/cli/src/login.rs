...
pub fn read_api_key_from_stdin() -> String {
    ...
    if stdin.is_terminal() {
        eprintln!(
            "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv GRADENCE_API_KEY | wecode login --with-api-key`."
        );
        std::process::exit(1);
    }
...
/// `wecode login` working in environments where device-code may be disabled/feature-gated.
