...
use codex_protocol::items::summarize_agent_message as summarize_core_agent_message;
...
use codex_protocol::protocol::DebugTraceAgentKind as CoreDebugTraceAgentKind;
use codex_protocol::protocol::DebugTraceEntry as CoreDebugTraceEntry;
use codex_protocol::protocol::DebugTraceLane as CoreDebugTraceLane;
use codex_protocol::protocol::DebugTraceLanePosition as CoreDebugTraceLanePosition;
use codex_protocol::protocol::DebugTraceRole as CoreDebugTraceRole;
use codex_protocol::protocol::DebugTraceSnapshot as CoreDebugTraceSnapshot;
...
v2_enum_from_core!(
    pub enum DebugTraceRole from CoreDebugTraceRole {
        System,
        Developer,
        User,
        Assistant,
...
v2_enum_from_core!(
    pub enum DebugTraceAgentKind from CoreDebugTraceAgentKind {
        Primary,
        SubAgent,
        Coordinator
    }
...
v2_enum_from_core!(
    pub enum DebugTraceLanePosition from CoreDebugTraceLanePosition {
        Left,
        Center,
        Right,
        Scroll
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct DebugTraceLane {
    pub agent_thread_id: String,
    pub agent_name: String,
    pub lane_index: u64,
    pub lane_position: DebugTraceLanePosition,
}

impl From<CoreDebugTraceLane> for DebugTraceLane {
    fn from(value: CoreDebugTraceLane) -> Self {
        Self {
            agent_thread_id: value.agent_thread_id.to_string(),
            agent_name: value.agent_name,
            lane_index: value.lane_index,
            lane_position: value.lane_position.into(),
        }
    }
}
...
pub struct DebugTraceEntry {
    ...
    pub timestamp_rfc3339_sec: String,
    pub agent_thread_id: String,
    pub agent_name: String,
    pub agent_kind: DebugTraceAgentKind,
    pub role: DebugTraceRole,
    pub source_event: String,
    pub content: String,
    #[serde(skip_serializing_if = "Option::is_none")]
    #[ts(optional)]
    pub lane_index: Option<u64>,
}

impl From<CoreDebugTraceEntry> for DebugTraceEntry {
    fn from(value: CoreDebugTraceEntry) -> Self {
        Self {
            conversation_id: value.conversation_id.to_string(),
            entry_seq: value.entry_seq,
            timestamp_unix_sec: value.timestamp_unix_sec,
            timestamp_rfc3339_sec: value.timestamp_rfc3339_sec,
            agent_thread_id: value.agent_thread_id.to_string(),
            agent_name: value.agent_name,
            agent_kind: value.agent_kind.into(),
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct DebugTraceSnapshot {
    pub schema_version: String,
    pub conversation_id: String,
    pub generated_at_unix_sec: i64,
    pub generated_at_rfc3339_sec: String,
    pub lanes: Vec<DebugTraceLane>,
    pub entries: Vec<DebugTraceEntry>,
}

impl From<CoreDebugTraceSnapshot> for DebugTraceSnapshot {
    fn from(value: CoreDebugTraceSnapshot) -> Self {
        Self {
            schema_version: value.schema_version,
            conversation_id: value.conversation_id.to_string(),
            generated_at_unix_sec: value.generated_at_unix_sec,
            generated_at_rfc3339_sec: value.generated_at_rfc3339_sec,
        ...
        match value {
            ...
            CoreTurnItem::AgentMessage(agent) => {
                ...
                ThreadItem::AgentMessage {
                    id: agent.id,
                    summary: summarize_core_agent_message(&text),
                    text,
                }
            }
...
mod tests {
    ...
    fn core_turn_item_into_thread_item_converts_supported_variants() {
        ...
        assert_eq!(
            ...
            ThreadItem::AgentMessage {
                ...
                summary: "已经回复用户：Hello world".to_string(),
            }
        );
    ...
    #[test]
    fn debug_trace_snapshot_converts_to_v2_shape() {
        let thread_id = codex_protocol::ThreadId::new();
        let core = CoreDebugTraceSnapshot {
            schema_version: "debug_trace_v1".to_string(),
            conversation_id: thread_id,
            generated_at_unix_sec: 10,
            generated_at_rfc3339_sec: "1970-01-01T00:00:10+00:00".to_string(),
            lanes: vec![CoreDebugTraceLane {
                agent_thread_id: thread_id,
                agent_name: "wmj-assistant".to_string(),
                lane_index: 1,
                lane_position: CoreDebugTraceLanePosition::Center,
            }],
            entries: vec![CoreDebugTraceEntry {
                conversation_id: thread_id,
                entry_seq: 1,
                timestamp_unix_sec: 10,
                timestamp_rfc3339_sec: "1970-01-01T00:00:10+00:00".to_string(),
                agent_thread_id: thread_id,
                agent_name: "wmj-assistant".to_string(),
                agent_kind: CoreDebugTraceAgentKind::Primary,
                role: CoreDebugTraceRole::Assistant,
        ...
        let v2 = DebugTraceSnapshot::from(core);
        assert_eq!(v2.schema_version, "debug_trace_v1");
        assert_eq!(v2.conversation_id, thread_id.to_string());
        assert_eq!(v2.lanes[0].lane_position, DebugTraceLanePosition::Center);
        assert_eq!(v2.entries[0].role, DebugTraceRole::Assistant);
    }

    #[test]
    fn thread_item_debug_trace_snapshot_serializes() {
        let item = ThreadItem::DebugTraceSnapshot {
            id: "debug-trace-1".to_string(),
            trace: DebugTraceSnapshot {
                schema_version: "debug_trace_v1".to_string(),
                conversation_id: "thread-1".to_string(),
                generated_at_unix_sec: 10,
                generated_at_rfc3339_sec: "1970-01-01T00:00:10+00:00".to_string(),
                lanes: vec![DebugTraceLane {
                    agent_thread_id: "thread-1".to_string(),
                    agent_name: "wmj-assistant".to_string(),
                    lane_index: 0,
                    lane_position: DebugTraceLanePosition::Center,
                }],
        ...
        assert_eq!(json["type"], "debugTraceSnapshot");
        assert_eq!(json["trace"]["lanes"][0]["lanePosition"], "center");
    }
}
