...
pub enum Op {
    ...
    UserInput {
        ...
        items: Vec<UserInput>,
        /// Origin of this input, used to distinguish human-entered input from
        /// agent-to-agent injected messages.
        #[serde(default, skip_serializing_if = "UserInputOrigin::is_user")]
...
pub enum EventMsg {
    ...
    /// Lightweight informational message for the user, typically shown in a
    /// subtle style in the UI.
    ...
    /// Work-summary update captured from an agent tool invocation.
    AgentWorkSummary(AgentWorkSummaryEvent),
    /// Canonical multi-agent debug trace snapshot update.
    DebugTraceSnapshot(DebugTraceSnapshotEvent),
}
...
impl From<AgentWorkSummaryEvent> for EventMsg {
    fn from(event: AgentWorkSummaryEvent) -> Self {
        EventMsg::AgentWorkSummary(event)
    }
}

impl From<DebugTraceSnapshotEvent> for EventMsg {
    fn from(event: DebugTraceSnapshotEvent) -> Self {
        EventMsg::DebugTraceSnapshot(event)
    }
}
...
impl fmt::Display for SpawnedAgentType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let value = match self {
            SpawnedAgentType::Default => "default",
            SpawnedAgentType::Worker => "worker",
            SpawnedAgentType::Explorer => "explorer",
            SpawnedAgentType::Verifier => "verifier",
            SpawnedAgentType::Coordinator => "coordinator",
        };
        f.write_str(value)
    }
}
        ...
        match self {
            ...
            SubAgentSource::ThreadSpawn {
                ...
                if let Some(agent_type) = agent_type {
                    write!(f, "thread_spawn_{parent_thread_id}_d{depth}_{agent_type}")
                } else {
...
pub struct CollabWaitingBeginEvent {
    ...
    pub targets: Vec<CollabWaitTargetEvent>,
}
...
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum CollabWaitLifecycleState {
    Pending,
    Running,
    Completed,
    TimedOut,
...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq, JsonSchema, TS)]
pub struct AgentWorkSummaryEvent {
    pub thread_id: ThreadId,
...
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum DebugTraceLanePosition {
    Left,
    Center,
    Right,
    Scroll,
...
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum DebugTraceContextChannel {
...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq, JsonSchema, TS)]
pub struct DebugTraceContextSection {
    pub conversation_id: ThreadId,
    pub turn_id: String,
    pub agent_thread_id: ThreadId,
    pub agent_name: String,
    pub agent_kind: DebugTraceAgentKind,
    pub channel: DebugTraceContextChannel,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub request_item_index: Option<u64>,
}
...
pub struct DebugTraceSnapshot {
    ...
    #[serde(default)]
    pub context_sections: Vec<DebugTraceContextSection>,
}

#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq, JsonSchema, TS)]
pub struct DebugTraceSnapshotEvent {
    pub conversation_id: ThreadId,
    pub snapshot: DebugTraceSnapshot,
}
