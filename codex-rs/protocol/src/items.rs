...
pub const AGENT_MESSAGE_SUMMARY_MAX_WORDS: usize = 40;
pub const AGENT_MESSAGE_SUMMARY_PREFIX: &str = "已经回复用户：";

pub fn summarize_agent_message(text: &str) -> String {
    let truncated = text
        .split_whitespace()
        .take(AGENT_MESSAGE_SUMMARY_MAX_WORDS)
        .collect::<Vec<_>>()
        .join(" ");
    format!("{AGENT_MESSAGE_SUMMARY_PREFIX}{truncated}")
}
        ...
        self.content
            ...
            .map(|c| match c {
                AgentMessageContent::Text { text } => EventMsg::AgentMessage(AgentMessageEvent {
                    message: text.clone(),
                    summary: summarize_agent_message(text),
                }),
            })
            .collect()
    }
...
#[cfg(test)]
mod tests {
    use super::AGENT_MESSAGE_SUMMARY_PREFIX;
    use super::summarize_agent_message;

    #[test]
    fn summarize_agent_message_keeps_short_text() {
        assert_eq!(
            summarize_agent_message("one two three"),
            format!("{AGENT_MESSAGE_SUMMARY_PREFIX}one two three")
        );
    }

    #[test]
    fn summarize_agent_message_truncates_to_first_40_words() {
        let text = (1..=45)
            .map(|i| format!("w{i}"))
            .collect::<Vec<_>>()
            .join(" ");
        let expected = (1..=40)
            ...
            .join(" ");
        assert_eq!(
            summarize_agent_message(&text),
            format!("{AGENT_MESSAGE_SUMMARY_PREFIX}{expected}")
        );
    }
}
