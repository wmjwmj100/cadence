    ...
    if !resp.status().is_success() {
        ...
        if status == StatusCode::NOT_FOUND {
            return Err(io::Error::new(
                ...
                "device code login is not enabled for this wecode server. Use the browser login or verify the server URL.",
            ));
        }
...
fn print_device_code_prompt(verification_url: &str, code: &str) {
    ...
    println!(
        "\nWelcome to wecode [v{ANSI_GRAY}{version}{ANSI_RESET}]\n{ANSI_GRAY}Gradence's command-line coding agent{ANSI_RESET}\n\
...
pub async fn request_device_code(opts: &ServerOptions) -> std::io::Result<DeviceCode> {
    ...
    Ok(DeviceCode {
        verification_url: format!("{base_url}/wecode/device"),
