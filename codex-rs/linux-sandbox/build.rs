...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=CODEX_DISABLE_VENDORED_BWRAP");
    ...
    if vendored_bwrap_disabled() {
        return;
    }

    if let Err(err) = try_build_vendored_bwrap() {
        println!(
            "cargo:warning=codex-linux-sandbox: vendored bubblewrap disabled for this build: {err}"
        );
    }
}

fn vendored_bwrap_disabled() -> bool {
    matches!(
        env::var("CODEX_DISABLE_VENDORED_BWRAP")
            .unwrap_or_default()
            .to_ascii_lowercase()
            .as_str(),
        "1" | "true" | "yes" | "on"
...
"#,
    ...
    build
        .try_compile("build_time_bwrap")
        .map_err(|err| format!("failed to compile vendored bubblewrap: {err}"))?;
